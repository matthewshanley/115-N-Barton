...
// ── Storage keys ───────────────────────────────────────────────────────────
const CRM_KEY   = "ecg-crm-v3";
const TASK_KEY  = "ecg-tasks-v1";
const MILE_KEY  = "ecg-miles-v1";

// ── Status colours ─────────────────────────────────────────────────────────
const LP_STAT_COL = {
  "Deck sent":B.blue,"Data room accessed":B.sage,"In conversation":B.sage,
  "Soft commit":B.gold,"Committed":"#2a6b3f","Passed":B.danger,
};
const LN_STAT_COL = {
  "Not contacted":B.muted,"Outreach sent":B.blue,"Term sheet requested":B.gold,
  "Term sheet received":B.gold,"In diligence":B.sage,"Committed":"#2a6b3f","Passed":B.danger,
};
...
const btn = (ghost=false) => ({
  fontSize:11,padding:"7px 16px",borderRadius:4,cursor:"pointer",
  letterSpacing:"0.06em",textTransform:"uppercase",fontFamily:FONT,fontWeight:600,
  background:ghost?"transparent":B.navy,color:ghost?B.navy:B.white,
  border:ghost?`1px solid ${B.navy}`:"none",
});
...
// ── localStorage helpers (replaces window.storage for standalone deploy) ───
const store = {
...
function Dashboard({contacts,tasks,miles}){
  ...
  const GANTT_START=new Date("2025-07-01"),GANTT_END=new Date("2027-07-01");
  const ganttTotal=GANTT_END-GANTT_START;
  const toPct=d=>((new Date(d)-GANTT_START)/ganttTotal)*100;
  const nowPct=Math.min(100,Math.max(0,((today-GANTT_START)/ganttTotal)*100));
    ...
    <div style={{padding:"1.25rem 0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem",marginBottom:"1rem"}}>
        <div style={card}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{height:6,background:B.light,borderRadius:3,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${nowPct}%`,background:B.blue,borderRadius:3}}/>
            </div>
          </div>
          {miles.slice(0,6).map(m=>{
            const s=new Date(m.start),e=new Date(m.end);
            const left=toPct(m.start),width=((e-s)/ganttTotal)*100;
            const color=phaseColors[m.phase]||B.muted;
            return(
              <div key={m.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                ...
                <div style={{flex:1,height:6,background:B.light,borderRadius:3,position:"relative"}}>
                  <div style={{position:"absolute",left:`${left}%`,width:`${Math.max(width,2)}%`,height:"100%",background:color,borderRadius:3}}/>
                </div>
              </div>
            );
          })}
        ...
        <div style={card}>
          ...
          {urgentTasks.length===0&&<div style={{fontSize:13,color:B.muted}}>All caught up.</div>}
          {urgentTasks.map(t=>(
      ...
      <div style={{padding:"1rem 0"}}>
        ...
        <div style={card}>
          <div style={{display:"flex",gap:14,marginBottom:"1rem",alignItems:"flex-start"}}>
            ...
            <div style={{display:"flex",alignItems:"center",fontSize:12,color:statCol(c.status),fontWeight:600}}>
              <Pip color={statCol(c.status)}/>{c.status}
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px 20px",marginBottom:"1rem",paddingTop:"1rem",borderTop:`1px solid ${B.light}`}}>
            ...
            {c.type==="Lender"&&<><Field label="Projected loan" value={fmt$(c.projectedLoanAmount)}/><Field label="Loan type" value={c.loanType}/><Field label="LTC appetite" value={c.ltcAppetite?`${c.ltcAppetite}%`:null}/><Field label="Loan range" value={(c.minLoanSize||c.maxLoanSize)?`${fmt$(c.minLoanSize)} – ${fmt$(c.maxLoanSize)}`:null}/><Field label="Geographies" value={c.geographies}/><Field label="Deals done" value={c.dealsDone}/></>}
          </div>
  ...
  if(view==="form"){
    const F=({label,field,type="text",opts,span})=>(
      <div style={span?{gridColumn:"span 2"}:{}}>
        ...
        {opts
          ?<select value={form[field]||""} onChange={e=>setForm(f=>({...f,[field]:e.target.value}))} style={inputStyle}>{opts.map(o=><option key={o}>{o}</option>)}</select>
    ...
    return(
      <div style={{padding:"1rem 0"}}>
        <div style={{display:"flex",gap:8,marginBottom:"1rem"}}>
          <button onClick={()=>setView(sel?"detail":"list")} style={btn(true)}>Cancel</button>
        </div>
        <div style={card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px 16px"}}>
            ...
            {form.type==="Lender"&&<>
              ...
              <F label="Min loan ($)" field="minLoanSize" type="number"/>
              <F label="Max loan ($)" field="maxLoanSize" type="number"/>
              <F label="LTC appetite (%)" field="ltcAppetite" type="number"/>
              <F label="Geographies" field="geographies"/>
          ...
          <div style={{display:"flex",gap:8,marginTop:"1rem"}}>
            <button onClick={submit} style={btn()}>Save contact</button>
          </div>
        </div>
      </div>
    );
  ...
  return(
    <div style={{padding:"1rem 0"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:"1rem",flexWrap:"wrap"}}>
        ...
        <select value={sf} onChange={e=>setSf(e.target.value)} style={{...inputStyle,width:"auto"}}>
        ...
        :<div style={{display:"flex",flexDirection:"column",gap:6}}>
          {visible.map(c=>(
            <div key={c.id} onClick={()=>openDetail(c)} style={{...card,cursor:"pointer",display:"flex",alignItems:"center",gap:12,padding:"10px 14px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",fontSize:11,color:statCol(c.status),fontWeight:600,flexShrink:0,whiteSpace:"nowrap"}}>
                <Pip color={statCol(c.status)}/>{c.status}
              </div>
            </div>
          ))}
...
// ══════════════════════════════════════════════════════════════════════════
const phaseColors={"Initiation":B.navy,"Planning":B.sage,"Execution":B.blue,"Go Live":B.gold};
const GANTT_START=new Date("2025-07-01");
const GANTT_END=new Date("2027-07-01");
const ganttTotal=GANTT_END-GANTT_START;
const toPct=d=>((new Date(d)-GANTT_START)/ganttTotal)*100;
const widthPct=(s,e)=>Math.max(((new Date(e)-new Date(s))/ganttTotal)*100,1);

const QUARTERS=[];
for(let y=2025;y<=2027;y++){
  for(let q=0;q<4;q++){
    const d=new Date(y,q*3,1);
    if(d>=GANTT_START&&d<=GANTT_END)QUARTERS.push({label:`Q${q+1} ${y}`,pct:toPct(d)});
  }
}
    ...
    <div style={{padding:"1rem 0"}}>
      <div style={{fontSize:11,color:B.muted,letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:"1rem",display:"flex",gap:16,flexWrap:"wrap"}}>
        {Object.entries(phaseColors).map(([ph,col])=>(
          <span key={ph} style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:10,height:10,borderRadius:2,background:col,display:"inline-block"}}/>{ph}</span>
        ))}
      </div>
      <div style={{...card,overflowX:"auto"}}>
        <div style={{display:"flex",marginBottom:8,marginLeft:180,position:"relative",height:20}}>
          {QUARTERS.map(q=>(
            <div key={q.label} style={{position:"absolute",left:`${q.pct}%`,fontSize:10,color:B.muted,letterSpacing:"0.04em",whiteSpace:"nowrap",transform:"translateX(-50%)"}}>{q.label}</div>
          ))}
        </div>
          ...
          {miles.map(m=>(
            <div key={m.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}} onClick={()=>{setEditing(m.id);setForm({...m});}}>
              ...
              <div style={{flex:1,height:20,background:B.light,borderRadius:4,position:"relative",cursor:"pointer"}}>
                <div style={{position:"absolute",left:`${Math.max(0,toPct(m.start))}%`,width:`${widthPct(m.start,m.end)}%`,height:"100%",background:phaseColors[m.phase]||B.muted,borderRadius:4,opacity:0.85}}/>
      ...
      {editing&&(()=>{
        const m=miles.find(x=>x.id===editing);
        return(
            ...
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:12}}>
              ...
              <div><label style={labelStyle}>Phase</label>
                <select value={form.phase||""} onChange={e=>setForm(f=>({...f,phase:e.target.value}))} style={inputStyle}>
                  {Object.keys(phaseColors).map(p=><option key={p}>{p}</option>)}
                </select>
              </div>
            </div>
  ...
  return(
    <div style={{padding:"1rem 0"}}>
      <div style={{display:"flex",gap:8,marginBottom:"1rem",flexWrap:"wrap",alignItems:"center"}}>
        ...
        <select value={ownerF} onChange={e=>setOwnerF(e.target.value)} style={{...inputStyle,width:"auto"}}>
        ...
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(2,29,43,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
          <div style={{...card,width:480,maxWidth:"90vw",maxHeight:"85vh",overflowY:"auto"}}>
            <div style={{fontSize:13,fontWeight:700,color:B.navy,letterSpacing:"0.05em",textTransform:"uppercase",marginBottom:"1rem"}}>
              {tasks.find(t=>t.id===form.id)?"Edit task":"New task"}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px 14px"}}>
              ...
              {[["Owner","owner","select",OWNERS],["Priority","priority","select",PRIORITIES],["Status","status","select",TASK_STATUSES],["Due date","due","date",null]].map(([l,f,type,opts])=>(
                <div key={f}>
                  ...
                  {opts
                    ?<select value={form[f]||""} onChange={e=>setForm(fm=>({...fm,[f]:e.target.value}))} style={inputStyle}>{opts.map(o=><option key={o}>{o}</option>)}</select>
                    :<input type={type} value={form[f]||""} onChange={e=>setForm(fm=>({...fm,[f]:e.target.value}))} style={inputStyle}/>}
                </div>
              ))}
              <div style={{gridColumn:"span 2"}}>
                <label style={labelStyle}>Notes</label>
                <textarea value={form.notes||""} onChange={e=>setForm(fm=>({...fm,notes:e.target.value}))} rows={3} style={{...inputStyle,resize:"vertical"}}/>
              </div>
            </div>
...
export default function App(){
  ...
  if(!loaded)return <div style={{fontFamily:FONT,padding:"3rem",color:B.muted,textAlign:"center",fontSize:14,letterSpacing:"0.05em"}}>Loading ECG Portal...</div>;
  ...
  return(
    <div style={{fontFamily:FONT,background:B.offwhite,minHeight:"100vh"}}>
      <div style={{background:B.navy,padding:"0 2rem",display:"flex",alignItems:"center",gap:0}}>
        ...
        <div style={{flex:1}}/>
        {saving&&<span style={{fontSize:10,color:"rgba(255,255,255,0.4)",letterSpacing:"0.05em"}}>Saving...</span>}
      </div>
